...
function calculateCorrelation(referenceImage, assessmentImage) {
    ...
    var xbar = referenceImage.reduceRegion(ee.Reducer.mean()).values();
    var ybar = assessmentImage.reduceRegion(ee.Reducer.mean()).values();
    ...
    var correlationNumerator = ee.Array(xCentered.multiply(yCentered).reduceRegion(ee.Reducer.sum()).values());
    
    var xSum = xCentered.pow(2).reduceRegion(ee.Reducer.sum()).values();
    var ySum = yCentered.pow(2).reduceRegion(ee.Reducer.sum()).values();
  ...
  function calculateContrast(referenceImage, assessmentImage) {
    var xStdDev = ee.Array(referenceImage.reduceRegion(ee.Reducer.stdDev()).values());
    var yStdDev = ee.Array(assessmentImage.reduceRegion(ee.Reducer.stdDev()).values());
    var xVar = ee.Array(referenceImage.reduceRegion(ee.Reducer.variance()).values());
    var yVar = ee.Array(assessmentImage.reduceRegion(ee.Reducer.variance()).values());
  ...
  exports.calculate = function(referenceImage, assessmentImage) {
    ...
    referenceImage = referenceImage.resample("bicubic").reproject(assessmentImage.projection());
