 ...
 */
function sharpen(sharpenable, pan, method, args) {
  var fn = _methods[method || "SFIM"];

  if (fn == null)
    throw new Error(
      "Method must be one of [" + exports.methods + "], not `" + method + "`."
    );
 ...
 */
function quality(original, modified, metric, args) {
  var fn = _metrics[metric];

  if (fn == null)
    throw new Error(
      "Metric must be one of [" + exports.metrics + "], not `" + metric + "`."
    );
